...
function CardOCRModal({open,onClose,onImport}) {
  ...
  async function handleFile(file) {
    ...
    try {
      ...
      const resp = await fetch("/api/ocr",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({base64,mediaType:file.type})});
      const data = await resp.json();
      if(data.error) throw new Error(data.error);
      setItems(data.items.map(p=>({...p,id:uid(),cat:PERSONAL_EXPENSE_CATS[0],tab:"personal",source:"salary",isShared:false,eventType:"daily",selected:true,editing:false})));
      setStep("review");
    } catch(e) { setError("내역을 읽지 못했어요."); }
    setLoading(false);
  }
  ...
  function doImport(){
    onImport(items.filter(x=>x.selected));
    reset(); onClose();
  }
  ...
  return (
    <BottomSheet open={open} onClose={()=>{reset();onClose();}} title={step==="upload"?"📸 카드 내역 가져오기":"내역 확인 및 수정"}>
      {step==="upload" && (
        <>
          <div style={{fontSize:13,color:"#aaa",marginBottom:20,lineHeight:1.7}}>카드사 앱의 결제 내역을 캡처해서 올리면 AI가 자동으로 읽어드려요</div>
      ...
      {step==="review" && (
        <>
          <div style={{fontSize:13,color:"#aaa",marginBottom:16}}>{items.filter(x=>x.selected).length}개 선택됨</div>
          {items.map(item=>(
            <div key={item.id} style={{borderRadius:16,border:`2px solid ${item.selected?"#2D7DD2":"#E8ECF0"}`,padding:"14px",marginBottom:10,background:item.selected?"#F8FBFF":"#FAFAFA"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:item.editing?12:0}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontWeight:700,fontSize:14}}>{item.rawName}</div>
                  <div style={{fontSize:12,color:"#aaa"}}>{item.date} · <span style={{fontWeight:700,color:"#E94F37"}}>{fmt(item.amount)}</span></div>
                </div>
                <button onClick={()=>updateItem(item.id,{editing:!item.editing})} style={{background:"none",border:"1px solid #eee",borderRadius:8,cursor:"pointer",fontSize:12,color:"#aaa",padding:"4px 8px"}}>{item.editing?"접기":"수정 ✏️"}</button>
              </div>
              {item.editing&&(
                <div style={{borderTop:"1px solid #F0F2F5",paddingTop:12,marginTop:4}}>
                  <div style={{marginBottom:10}}><Lbl>실제 내용 메모</Lbl><Inp value={item.memo} placeholder="예: 스타벅스 라떼" onChange={e=>updateItem(item.id,{memo:e.target.value})}/></div>
                  ...
                  <div style={{marginBottom:10}}><Lbl>개인 / 공유</Lbl><div style={{display:"flex",gap:8}}><KBtn label="👤 개인" active={item.tab==="personal"} onClick={()=>updateItem(item.id,{tab:"personal"})} color="#1A1D23"/><KBtn label="💑 공유" active={item.tab==="shared"} onClick={()=>updateItem(item.id,{tab:"shared"})} color="#2D7DD2"/></div></div>
                  {item.tab==="personal"&&<>
                    <div style={{marginBottom:10}}><Lbl>어디서?</Lbl><div style={{display:"flex",gap:8}}><KBtn label="💰 월급" active={item.source==="salary"} onClick={()=>updateItem(item.id,{source:"salary"})} color="#E94F37"/><KBtn label="🎁 월급 외" active={item.source==="extra"} onClick={()=>updateItem(item.id,{source:"extra"})} color="#2D7DD2"/></div></div>
                    <button onClick={()=>updateItem(item.id,{isShared:!item.isShared})} style={{width:"100%",padding:"9px",borderRadius:10,border:"2px solid",borderColor:item.isShared?"#2D7DD2":"#E8ECF0",background:item.isShared?"#EEF6FF":"#fff",color:item.isShared?"#2D7DD2":"#bbb",fontWeight:700,fontSize:13,cursor:"pointer",marginBottom:10}}>{item.isShared?"💑 공유로 집계":"공유 지출로 표시"}</button>
                  </>}
                  {item.tab==="shared"&&<div style={{marginBottom:10}}><Lbl>지출 유형</Lbl><div style={{display:"flex",gap:8}}><KBtn label="🗓️ 일상" active={item.eventType==="daily"} onClick={()=>updateItem(item.id,{eventType:"daily"})} color="#2D7DD2"/><KBtn label="🎉 이벤트" active={item.eventType==="event"} onClick={()=>updateItem(item.id,{eventType:"event"})} color="#F26419"/></div></div>}
                  <div><Lbl>카테고리</Lbl><div style={{display:"flex",flexWrap:"wrap",gap:5}}>{(item.tab==="personal"?PERSONAL_EXPENSE_CATS:item.eventType==="event"?SHARED_EVENT_CATS:SHARED_DAILY_CATS).map(c=><button key={c} onClick={()=>updateItem(item.id,{cat:c})} style={{padding:"5px 10px",borderRadius:16,border:"none",cursor:"pointer",background:item.cat===c?"#1A1D23":"#F0F2F5",color:item.cat===c?"#fff":"#666",fontSize:11,fontWeight:600}}>{c}</button>)}</div></div>
                </div>
              )}
            </div>
          ))}
        ...
        <>
          ...
          {items.map(r=>(
            <div key={r.id} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 0",borderBottom:"1px solid #F0F2F5"}}>
              <div style={{flex:1}}><div style={{fontWeight:700,fontSize:14}}>{r.name}</div><div style={{fontSize:12,color:"#aaa",marginTop:2}}>매월 {r.day}일 · {fmt(r.amount)}</div></div>
...
function exportExcel(personal, shared, month) {
  ...
  const pRows=personal.filter(e=>!month||monthOf(e.date)===month).map(e=>({날짜:e.date,구분:e.kind==="expense"?"지출":e.kind==="income_salary"?"월급":e.kind==="income_extra"?"월급외":"저축",금액:e.amount,카테고리:e.cat,재원:e.source==="extra"?"월급외":"월급",공유:e.isShared?"공유":"개인",메모:e.memo||""}));
  ...
  XLSX.writeFile(wb,`가계부_${month||"전체"}.xlsx`);
}
